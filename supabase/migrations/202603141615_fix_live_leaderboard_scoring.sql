-- Migration: Fix FB Lega Live Leaderboard Scoring
-- Description: Updates the live leaderboard RPC to use the league's custom scoring rules instead of a hardcoded value for X.
                        ...
                        SELECT COALESCE(SUM(
                            CASE 
                                WHEN pp.predictions[i] = v_results[i] THEN 
                                    COALESCE((v_league_rules->>v_results[i])::INTEGER, 1) +
