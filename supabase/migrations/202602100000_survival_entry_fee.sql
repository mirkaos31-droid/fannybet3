-- Migration: Add configurable entry fee to Survival Seasons and automate winner logic

-- 1. Add entry_fee column to survival_seasons (Default 2 as requested)
ALTER TABLE public.survival_seasons 
ADD COLUMN IF NOT EXISTS entry_fee INTEGER DEFAULT 2;

-- 2. Update start_new_survival_season to accept entry_fee
...
-- 3. Update join_survival to use the season's specific entry_fee
...
BEGIN
  ...
  v_cost := v_season.entry_fee; -- Use the configured fee
  ...
  -- 5. Register Player
  INSERT INTO survival_players (season_id, user_id, status)
